...
import { BarChart4, Calendar, Clock, DollarSign, ClipboardList, CreditCard, Banknote, ArrowDownToLine, CheckCircle, AlertCircle, XCircle } from "lucide-react";
...
const getNextWednesday = () => {
  ...
  return nextWednesday.toLocaleDateString('pt-BR');
};
...
export default function AnalyticsPage() {
  ...
  // Handler para mudança no campo PIX Key
  const handlePixKeyChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    
    if (pixKeyType === 'phone') {
      // Formatar automaticamente se for telefone
  ...
  // Handler para mudança do tipo de chave PIX
  const handlePixKeyTypeChange = (value: string) => {
    setPixKeyType(value);
    setPixKey(''); // Limpa o campo quando muda o tipo
  };
  
  // Buscar dados de análise
  const { data: analytics, isLoading, refetch: refetchAnalytics } = useQuery<ProviderAnalytics>({
  ...
  const withdrawalMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      refetchAnalytics(); // Refresh analytics to update available balance
    },
    onError: (error: Error) => {
      toast({
  ...
  const handleWithdrawalSubmit = () => {
    ...
    if (pixKeyType === 'phone' && !isValidPhoneNumber(pixKey)) {
      toast({
        title: "Telefone inválido",
        description: "Por favor, digite um número de telefone válido com DDD. Exemplo: (11) 99999-9999",
        variant: "destructive",
      });
      return;
    }
    ...
    if (amount > availableBalance) {
      toast({
        ...
        description: `Você só pode sacar até ${formatCurrencyFromReais(availableBalance)}. Saldo disponível baseado em agendamentos pagos menos saques já solicitados.`,
        variant: "destructive",
      });
      return;
    }
  ...
  // Check if today is Wednesday (day 3 in JavaScript, where Sunday = 0)
  const today = new Date();
  const isWednesday = today.getDay() === 3;
  
  // Check if user already made a request this Wednesday
  const hasRequestThisWednesday = withdrawalRequests?.some((request: any) => {
    const requestDate = new Date(request.requestedAt);
    return requestDate.getDay() === 3 && 
                  ...
                  <Card className="border border-[#58c9d1]/20 bg-white shadow-xl shadow-[#58c9d1]/10 rounded-2xl overflow-hidden relative group hover:shadow-2xl hover:shadow-[#58c9d1]/15 transition-all duration-500">
                    ...
                    <CardContent className="p-4 flex flex-col items-center justify-center">
                      ...
                      <p className="text-2xl font-medium text-[#58c9d1]">
                        {analytics?.totalRevenue ? formatCurrencyFromReais(analytics.totalRevenue) : "R$ 0,00"}
                      </p>
                    </CardContent>
                  </Card>
                  
                  <Card className="border border-[#58c9d1]/20 bg-white shadow-xl shadow-[#58c9d1]/10 rounded-2xl overflow-hidden relative group hover:shadow-2xl hover:shadow-[#58c9d1]/15 transition-all duration-500">
                    <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-emerald-400/50 via-emerald-500 to-emerald-400/50"></div>
                    <CardContent className="p-4 flex flex-col items-center justify-center">
                      <div className="p-2 bg-gradient-to-r from-emerald-500 to-emerald-600 rounded-xl shadow-lg mb-2">
                        <Banknote className="h-6 w-6 text-white" />
                      </div>
                      <p className="text-sm text-neutral-500 font-medium">Saldo Disponível</p>
                      <p className="text-2xl font-medium text-emerald-600">
                        {analytics?.availableBalance ? formatCurrencyFromReais(analytics.availableBalance) : "R$ 0,00"}
                      </p>
                        ...
                        <div className="flex-1">
                          <div className="mb-3 p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200">
                            ...
                            <p className="text-xs text-green-600 mt-1">
                              Baseado em agendamentos pagos no período selecionado menos saques já solicitados
                            </p>
                          </div>
                          <p className="text-sm text-neutral-600 mb-2 font-medium">
                            📅 <strong>Informação importante:</strong> O saque é processado toda quinta-feira ou em até 48 horas e contém taxa de transações do banco.
                          </p>
                          <p className="text-xs text-neutral-500">
                            Solicite seu saque toda quarta-feira (apenas uma solicitação por dia) e receba o valor em sua conta PIX na quinta-feira ou em até 48 horas.
                          </p>
                          {!isWednesday && (
                            <p className="text-xs text-amber-600 mt-2 font-medium">
                              ⚠️ Saques só podem ser solicitados às quartas-feiras. Próxima data disponível: {getNextWednesday()}.
                            </p>
                          )}
                          {isWednesday && hasRequestThisWednesday && (
                            <p className="text-xs text-amber-600 mt-2 font-medium">
                              ⚠️ Você já fez uma solicitação de saque nesta quarta-feira.
                            </p>
                          )}
                        </div>
                        <Dialog open={withdrawalDialogOpen} onOpenChange={(open) => {
                          if (isWednesday && !hasRequestThisWednesday) {
                          ...
                          <DialogTrigger asChild>
                            <Button 
                              ...
                              size="lg"
                              disabled={!isWednesday || hasRequestThisWednesday}
                            >
                              <ArrowDownToLine className="h-5 w-5 mr-2" />
                              <span className="font-medium">
                                {!isWednesday ? "Disponível às Quartas" : 
                                 hasRequestThisWednesday ? "Já Solicitado Hoje" : "Solicitar Saque"}
                              </span>
                            </Button>
                          </DialogTrigger>
                              ...
                              <div>
                                <Label htmlFor="amount" className="text-sm font-medium text-neutral-700">
                                  ...
                                  <span className="text-xs text-gray-500 ml-2">
                                    Máximo: {analytics?.availableBalance ? formatCurrencyFromReais(analytics.availableBalance) : "R$ 0,00"}
                                  </span>
                                </Label>
                                <Input
                                  id="amount"
                                  ...
                                  step="0.01"
                                  min="0"
                                  max={analytics?.availableBalance || 0}
                                  value={withdrawalAmount}
                                  onChange={(e) => setWithdrawalAmount(e.target.value)}
                                  placeholder="0,00"
                                  className="mt-1 border-[#58c9d1]/30 focus:border-[#58c9d1]"
                                />
                                {parseFloat(withdrawalAmount) > (analytics?.availableBalance || 0) && withdrawalAmount && (
                                ...
                                <Input
                                  ...
                                  placeholder={
                                    pixKeyType === 'phone' 
                                      ...
                                      : pixKeyType === 'cnpj'
                                      ? 'Digite seu CNPJ: 00.000.000/0000-00'
                                      : pixKeyType === 'email'
                                      ? 'Digite seu e-mail: exemplo@email.com'
                                      : 'Digite sua chave aleatória'
                                  }
                                  className="mt-1 border-[#58c9d1]/30 focus:border-[#58c9d1]"
                                  maxLength={pixKeyType === 'phone' ? 15 : undefined}
                                />
                                {pixKeyType === 'phone' && pixKey && !isValidPhoneNumber(pixKey) && (
                                  <p className="text-xs text-red-500 mt-1">
                            ...
                            <ResponsiveContainer width="100%" height={300} className="mt-4">
                              ...
                              >
                                ...
                                <XAxis 
                                  ...
                                  interval={window.innerWidth < 768 ? 0 : 0} // Show all labels on mobile
                                  tick={{ fontSize: window.innerWidth < 768 ? 10 : 14 }}
                                  tickMargin={window.innerWidth < 768 ? 10 : 5}
                                />
                                <YAxis 
                                ...
                                <Tooltip 
                                  ...
                                  contentStyle={window.innerWidth < 768 ? { fontSize: '12px' } : {}}
                                />
                                <Bar dataKey="count" fill="#3b82f6" barSize={window.innerWidth < 768 ? 15 : 30} />
                              </BarChart>
                            </ResponsiveContainer>
                          ) : (
